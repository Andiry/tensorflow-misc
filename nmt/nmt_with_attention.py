...
class Decoder(tf.keras.Model):
  ...
  def call(self, x, hidden, enc_output):
    ...
    # Perform addition to calculate score
    hidden_with_time_axis = tf.expand_dims(hidden, 1)

    # score shape = [batchsize, max_length, 1]
    # score(ht, hs) = vt * tanh(W1ht + W2hs-)
    score = self.V(tf.nn.tanh(self.W1(enc_output) + self.W2(hidden_with_time_axis)))

    # attention weights shape = [batchsize, max_length, 1]
    attention_weights = tf.nn.softmax(score, axis=1)

    # context_vector shape after sum = [batchsize, hidden_size]
    context_vector = attention_weights * enc_output
    context_vector = tf.reduce_sum(context_vector, axis=1)

    # x shape == [batchsize, 1, embdding_dim]
    x = self.embedding(x)

    # x shape after concat = [batchsize, 1, embedding_dim + hidden_size]
    x = tf.concat([tf.expand_dims(context_vector, 1), x], axis = -1)

    # passing concated vector to GRU
...
def evaluate(sentence, encoder, decoder, inp_lang, targ_lang, max_length_inp, max_length_targ):
  ...
  for t in range(max_length_targ):
    ...
    result += targ_lang.idx2word[predict_id] + ' '

    if targ_lang.idx2word[predict_id] == '<end>':
